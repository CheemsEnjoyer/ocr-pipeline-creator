...
test("nested configuration preserves instructions and drops public descriptions through serialization", () => {
  ...
  assert.equal("public_description" in saved[0], false);
  assert.equal(saved[0].fields[0].type, "string");
  assert.equal("public_description" in saved[0].fields[0], false);
  assert.equal("public_description" in saved[0].fields[1], false);
