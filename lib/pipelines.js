...
export async function savePipeline(pipeline, savedId = null) {
  await refreshPipelines();
  if (state.error) throw new Error(state.error);
  const data = await request(savedId ? `/api/pipelines/${encodeURIComponent(savedId)}` : "/api/pipelines", json(savedId ? "PATCH" : "POST", pipeline));
...
export function describePipeline(pipeline) {
  const ocr = pipeline.source !== "scans" || !pipeline.ocr || pipeline.ocr.enabled === false ? "Прямое извлечение текста" : pipeline.ocr.provider === "litellm" ? `Vision · ${pipeline.ocr.model}` : `OCR-сервис · ${pipeline.ocr.url ?? ""}`;
  const config = pipeline.extraction;
  const promptEnabled = config && (config.prompt_enabled ?? config.mode === "prompt");
  const fieldsEnabled = config && (config.fields_enabled ?? config.mode === "fields");
  const extraction = fieldsEnabled ? `${config.fields.length} полей${promptEnabled ? " + промпт" : ""} · ${config.model}` : promptEnabled ? `Промпт · ${config.model}` : "Исходный текст";
