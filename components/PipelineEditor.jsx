...
export default function PipelineEditor({ initialPipeline = null, onCreateNew }) {
  ...
  const [executionMode, setExecutionMode] = useState(initialPipeline?.allow_sync === false && initialPipeline?.allow_async ? "async" : "sync");
  const allowSync = executionMode === "sync";
  const allowAsync = executionMode === "async";
  ...
  const promptAllowed = !(sourceType === "scans" && ocrMode === "litellm");
  ...
  const isValid = useMemo(() => {
    ...
    if (step === 3) {
      const ocrValid = sourceType !== "scans" || (ocrMode === "litellm" ? Boolean(ocrModel) && validTemperature(ocrTemperature) && visionPrompt.trim().length > 0 : /^https?:\/\//.test(ocrServiceUrl));
    ...
    return true;
  }, [ocrTemperature, extractionTemperature, step, name, allowSync, allowAsync, asyncConcurrency, sourceType, ocrMode, ocrModel, ocrServiceUrl, visionPrompt, skipExtraction, llmModel, maxTokens, promptEnabled, fieldsEnabled, prompt, fields]);
  ...
  const pipeline = {
    ...(savedId ? { id: savedId } : {}), name: name.trim(), priority: Number(priority), allow_sync: allowSync, allow_async: allowAsync, async_concurrency: Number(asyncConcurrency), source: sourceType,
    ocr: sourceType === "scans" ? (ocrMode === "litellm" ? { enabled: true, provider: "litellm", model: ocrModel, temperature: Number(ocrTemperature), prompt: visionPrompt } : { enabled: true, provider: "service", url: ocrServiceUrl || null, ...(Object.keys(serviceOptions).length ? { options: serviceOptions } : {}) }) : null,
  ...
  useEffect(() => {
    ...
    try { void Promise.resolve(context.registerTool({
      ...
      inputSchema: { type: "object", properties: { name: { type: "string" }, priority: { type: "integer", minimum: 0, maximum: 9 }, executionMode: { type: "string", enum: ["sync", "async"] }, asyncConcurrency: { type: "integer", minimum: 1, maximum: 30 }, source: { type: "string", enum: ["scans", "document"] }, extractionMode: { type: "string", enum: ["prompt", "fields"] }, promptEnabled: { type: "boolean" }, fieldsEnabled: { type: "boolean" }, model: { type: "string" }, maxTokens: { type: "integer", minimum: 1 } }, required: ["name", "source", "model", "maxTokens"], additionalProperties: false },
      annotations: { readOnlyHint: false, untrustedContentHint: false }, execute(input) { setName(input.name); setPriority(input.priority ?? 5); setExecutionMode(input.executionMode === "async" ? "async" : "sync"); setAsyncConcurrency(input.asyncConcurrency ?? 1); setSourceType(input.source); setPromptEnabled(input.promptEnabled ?? input.extractionMode === "prompt"); setFieldsEnabled(input.fieldsEnabled ?? input.extractionMode !== "prompt"); setLlmModel(input.model); setMaxTokens(input.maxTokens); setStep(3); return { configured: true, name: input.name }; },
  ...
  return <main className="app-shell">
    ...
    <div className="wizard-layout">
      ...
      <section className="question-area"><div className="progress-row"><span>Шаг {step} из 3</span><div><i style={{ width: `${Math.round(step / 3 * 100)}%` }}/></div><strong>{Math.round(step / 3 * 100)}%</strong></div><div className="question-card">
        {step === 1 && <StepName name={name} setName={setName} priority={priority} setPriority={setPriority} executionMode={executionMode} setExecutionMode={setExecutionMode} asyncConcurrency={asyncConcurrency} setAsyncConcurrency={setAsyncConcurrency}/>}
        ...
        {step === 3 && <div className="step-content"><StepHeading icon={Sparkles} kicker="ОБРАБОТКА ДОКУМЕНТА" title="Промпт и поля в одном этапе" copy="Включайте нужные действия независимо. Без промпта и полей результатом будет исходный текст."/>
          {sourceType === "scans" && <StepOcr temperature={ocrTemperature} setTemperature={setOcrTemperature} sourceType={sourceType} ocrMode={ocrMode} setOcrMode={setOcrMode} ocrModel={ocrModel} setOcrModel={setOcrModel} ocrServiceUrl={ocrServiceUrl} setOcrServiceUrl={setOcrServiceUrl} visionPrompt={visionPrompt} setVisionPrompt={setVisionPrompt} models={models} modelsLoading={modelsLoading} modelError={modelError}/>}
      ...
      <aside className="summary-panel"><div className="summary-head"><span>КОНФИГУРАЦИЯ</span><strong>{name.trim() || "Без названия"}</strong></div><SummaryRow number="01" label="Источник" value={sourceType === "scans" ? "Сканы и изображения" : "Цифровой документ"}/><SummaryRow number="02" label="Получение текста" value={sourceType !== "scans" ? "Прямое извлечение" : ocrMode === "litellm" ? (ocrModel || "Модель не выбрана") : (ocrServiceUrl || "Сервис не указан")}/><SummaryRow number="03" label="Извлечение данных" value={skipExtraction ? "Отключено — исходный текст" : fieldsEnabled ? `${fields.length} полей${promptEnabled ? " + промпт" : ""}` : "Свободный промпт"}/><SummaryRow number="04" label="LLM" value={skipExtraction ? "Не используется" : llmModel || "Модель не выбрана"}/><SummaryRow number="05" label="Режим запуска" value={allowAsync ? `Асинхронный · ${asyncConcurrency}` : "Синхронный"}/><SummaryRow number="06" label="Приоритет Celery" value={allowAsync ? `${PRIORITIES.find(([value]) => value === Number(priority))?.[1] ?? "Пользовательский"} · ${priority}` : "Не используется"}/><div className="token-summary"><span>{skipExtraction ? "Финальный результат" : "Лимит ответа"}</span><strong>{skipExtraction ? "Исходный текст" : `${Number(maxTokens).toLocaleString("ru-RU")} tokens`}</strong></div></aside>
    </div>
  </main>;
}

function StepName({ name, setName, priority, setPriority, executionMode, setExecutionMode, asyncConcurrency, setAsyncConcurrency }) {
  const allowAsync = executionMode === "async";
  return <div className="step-content">
    ...
    <div className="main-field"><Label id="execution-mode-label">Режим запуска</Label><RadioGroup aria-labelledby="execution-mode-label" value={executionMode} onValueChange={setExecutionMode} className="method-grid">
      <MethodCard value="sync" title="Синхронный запуск" copy="Запрос ждёт завершения обработки и сразу получает результат."/>
      <MethodCard value="async" title="Асинхронный запуск" copy="Документ обрабатывается в фоне. Результат запрашивается по ID задачи."/>
    </RadioGroup></div>
...
function PipelineSummary({ pipeline }) { return <div className="final-summary"><div><span>Режим запуска</span><strong>{[pipeline.allow_sync && "Синхронный", pipeline.allow_async && `Асинхронный · до ${pipeline.async_concurrency}`].filter(Boolean).join(" + ")}</strong></div><div><span>Приоритет Celery</span><strong>{pipeline.allow_async ? `${PRIORITIES.find(([value]) => value === pipeline.priority)?.[1] ?? pipeline.priority} · ${pipeline.priority}` : "Не используется"}</strong></div><div><span>Источник</span><strong>{pipeline.source === "scans" ? "Сканы / изображения" : "Цифровой документ"}</strong></div><div><span>OCR</span><strong>{pipeline.ocr && pipeline.ocr.enabled !== false ? (pipeline.ocr.model || pipeline.ocr.url) : "Не требуется"}</strong></div><div><span>Извлечение</span><strong>{pipeline.extraction && (pipeline.extraction.prompt_enabled || pipeline.extraction.fields_enabled) ? (pipeline.extraction.fields_enabled ? `${pipeline.extraction.fields.length} полей${pipeline.extraction.prompt_enabled ? " + промпт" : ""}` : "Промпт") : "Отключено"}</strong></div><div><span>LLM</span><strong>{pipeline.extraction && (pipeline.extraction.prompt_enabled || pipeline.extraction.fields_enabled) ? `${pipeline.extraction.model} · ${pipeline.extraction.max_tokens} tokens` : "Исходный текст"}</strong></div></div>; }
