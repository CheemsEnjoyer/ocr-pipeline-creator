...
export default function PipelineEditor({ initialPipeline = null, onCreateNew }) {
  ...
  if (created) return <main className="app-shell success-shell"><div className="success-card"><div className="success-icon"><Check size={28}/></div><p className="eyebrow">ПАЙПЛАЙН ГОТОВ</p><h1>{pipeline.name}</h1><p>Пайплайн сохранён. Его можно выбрать для обработки или отредактировать в боковом меню.</p><PipelineSummary pipeline={pipeline}/><div className="success-actions"><Button variant="outline" onClick={() => setCreated(false)}>Продолжить редактирование</Button><Button variant="outline" onClick={() => { if (onCreateNew) onCreateNew(); else window.location.assign("/"); }}>Создать ещё один</Button><Button asChild><Link href="/process">Загрузить документы<ArrowRight size={16}/></Link></Button></div></div></main>;
