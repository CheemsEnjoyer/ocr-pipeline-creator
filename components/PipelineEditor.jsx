...
export default function PipelineEditor({ initialPipeline = null, onCreateNew }) {
  ...
  const [promptEnabled, setPromptEnabled] = useState(initialPipeline?.extraction?.prompt_enabled ?? initialPipeline?.extraction?.mode === "prompt");
  ...
  const isValid = useMemo(() => {
    ...
    if (step === 3) {
      ...
      const fieldsValid = fields.length > 0 && fields.every((field) => field.name.trim()) && new Set(fields.map((field) => field.name.trim())).size === fields.length;
    ...
    return true;
  }, [ocrEnabled, ocrTemperature, extractionTemperature, step, name, pipelineId, sourceType, ocrMode, ocrModel, ocrServiceUrl, visionPrompt, skipExtraction, llmModel, maxTokens, promptEnabled, fieldsEnabled, prompt, fields]);
  ...
  const pipeline = {
    ...
    extraction: { mode: fieldsEnabled ? "fields" : "prompt", prompt_enabled: promptEnabled, fields_enabled: fieldsEnabled, model: llmModel, max_tokens: Number(maxTokens), temperature: Number(extractionTemperature), prompt, fields: fields.map(({ name, description }) => ({ name: name.trim(), description })) },
  };
  ...
  useEffect(() => {
    ...
    try { void Promise.resolve(context.registerTool({
      ...
      inputSchema: { type: "object", properties: { id: { type: "string", pattern: "^[a-zA-Z0-9_-]{1,80}$" }, name: { type: "string" }, source: { type: "string", enum: ["scans", "document"] }, extractionMode: { type: "string", enum: ["prompt", "fields"] }, promptEnabled: { type: "boolean" }, fieldsEnabled: { type: "boolean" }, model: { type: "string" }, maxTokens: { type: "integer", minimum: 1 } }, required: ["id", "name", "source", "model", "maxTokens"], additionalProperties: false },
      annotations: { readOnlyHint: false, untrustedContentHint: false }, execute(input) { if (!savedId) setPipelineId(input.id); setName(input.name); setSourceType(input.source); setPromptEnabled(input.promptEnabled ?? input.extractionMode === "prompt"); setFieldsEnabled(input.fieldsEnabled ?? input.extractionMode !== "prompt"); setLlmModel(input.model); setMaxTokens(input.maxTokens); setStep(3); return { configured: true, name: input.name }; },
...
function StepExtraction({ temperature, setTemperature, promptEnabled, setPromptEnabled, fieldsEnabled, setFieldsEnabled, prompt, setPrompt, fields, setFields, addField, llmModel, setLlmModel, maxTokens, setMaxTokens, models, modelsLoading, modelError }) {
  return <div className="step-content">
    <ToggleSection title="Промпт" copy="Свободная инструкция для получения текста. Вместе с полями задаёт правила их извлечения." checked={promptEnabled} onChange={setPromptEnabled}/>
    ...
    <ToggleSection title="Поля извлечения" copy="Результат в JSON по заданным полям. Можно использовать с промптом или отдельно." checked={fieldsEnabled} onChange={setFieldsEnabled}/>
    {fieldsEnabled && <><div className="fields-intro"><span>Поля результата</span><Button variant="outline" size="sm" onClick={addField}><Plus size={15}/>Добавить поле</Button></div><div className="fields-list">{fields.map((field, index) => <div className="parameter-card" key={field.id}><span className="field-number">{index + 1}</span><div><Input aria-label={`Название поля ${index + 1}`} value={field.name} onChange={(event) => setFields(fields.map((item) => item.id === field.id ? { ...item, name: event.target.value } : item))} placeholder="Название поля, например total"/><Textarea aria-label={`Описание поля ${index + 1}`} rows={2} value={field.description} onChange={(event) => setFields(fields.map((item) => item.id === field.id ? { ...item, description: event.target.value } : item))} placeholder="Что нужно извлечь и в каком формате"/></div><button className="remove-field" onClick={() => setFields(fields.filter((item) => item.id !== field.id))} aria-label="Удалить поле"><Trash2 size={16}/></button></div>)}</div></>}
