...
def create_router(app, storage, policy=integration_allowed):
    ...
    def documents(request: Request, page: int = Query(default=0, ge=0), pipeline_id: str | None = Query(default=None),
                  ...
                  search: str = Query(default="", max_length=200, description="Подстрока имени файла или ID документа, без учёта регистра")):
        ...
        with app.state.sessions() as session:
            ...
            needle = search.strip()
            if needle:
                # autoescape: % и _ из запроса ищутся буквально, а не как шаблон LIKE.
                query = query.where(or_(Document.filename.icontains(needle, autoescape=True), Document.id.icontains(needle, autoescape=True)))
