...
export default function Home() {
  ...
  const runPipeline = () => { setRunning(true); window.setTimeout(() => setRunning(false), 1800); };
  ...
  return <main className="app-shell">
    ...
    <section className="workspace">
      ...
      <aside className="settings-panel"><div className="settings-head"><div className={`step-icon step-${active}`}><ActiveIcon size={17}/></div><div><p className="panel-kicker">НАСТРОЙКИ БЛОКА</p><h2>{activeMeta.label}</h2></div><button className="icon-button" aria-label="Закрыть"><X size={18}/></button></div>
        ...
        {active==="llm"&&<Tabs defaultValue="fields" className="settings-tabs"><TabsList className="grid w-full grid-cols-2"><TabsTrigger value="prompt">Промпт</TabsTrigger><TabsTrigger value="fields">Параметры</TabsTrigger></TabsList><TabsContent value="prompt" className="settings-body"><div className="field-group"><Label>Инструкция для модели</Label><Textarea rows={8} value={prompt} onChange={(e)=>{setPrompt(e.target.value);setSaved(false)}}/><small className="counter">{prompt.length} / 4 000</small></div><div className="field-group"><Label>Модель</Label><Select defaultValue="balanced"><SelectTrigger><SelectValue/></SelectTrigger><SelectContent><SelectItem value="balanced">Сбалансированная</SelectItem><SelectItem value="fast">Быстрая</SelectItem><SelectItem value="smart">Максимальная точность</SelectItem></SelectContent></Select></div></TabsContent><TabsContent value="fields" className="settings-body"><p className="section-copy">Опишите данные, которые нужно получить из документа.</p><div className="field-list">{fields.map((field,index)=><div className="extract-field" key={field.id}><div className="field-index">{index+1}</div><div><Input value={field.name} onChange={(e)=>setFields(fields.map(item=>item.id===field.id?{...item,name:e.target.value}:item))}/><Textarea value={field.description} rows={2} onChange={(e)=>setFields(fields.map(item=>item.id===field.id?{...item,description:e.target.value}:item))}/></div><button aria-label="Дублировать поле" onClick={()=>setFields([...fields,{...field,id:Date.now()}])}><Copy size={15}/></button></div>)}</div><Button variant="outline" className="add-field" onClick={addField}><Plus size={16}/> Добавить параметр</Button></TabsContent></Tabs>}
    ...
    </section>{running&&<div className="run-toast"><LoaderCircle className="animate-spin" size={18}/><div><strong>Пайплайн запущен</strong><span>Обрабатываем {files.length||1} файл…</span></div></div>}
  </main>;
}
