...
export default function Home() {
    ...
    const [files, setFiles] = useState(["invoice_0241.pdf"]);
    ...
    const runPipeline = async () => {
        if (!selectedModel) {
            setRunMessage("Сначала подключите LiteLLM и выберите модель");
            return;
        }
        setRunning(true);
        setRunMessage("");
        try {
            ...
            if (!response.ok)
                throw new Error(payload.error || "LiteLLM не ответил");
            setRunMessage(`Готово · ${selectedModel}`);
        }
        catch (error) {
            setRunMessage(error instanceof Error ? error.message : "Ошибка LiteLLM");
        }
        finally {
            setRunning(false);
    ...
    useEffect(() => {
        ...
        fetch("/api/litellm/models", { signal: controller.signal })
            .then(async (response) => { const payload = await response.json(); if (!response.ok)
            throw new Error(payload.error || "Не удалось получить модели"); return payload.models ?? []; })
            ...
            setModelError(error instanceof Error ? error.message : "LiteLLM недоступен"); })
    ...
    useEffect(() => {
        ...
        const tool = {
            ...
            execute(input) {
                const value = input;
                if (!Array.isArray(value.fields) || value.fields.some((item) => !item.name?.trim() || !item.description?.trim()))
                    throw new Error("Укажите название и описание каждого параметра.");
...
function SettingSwitch({ title, copy, defaultChecked = false }) { return <div className="switch-row"><div><strong>{title}</strong><span>{copy}</span></div><Switch defaultChecked={defaultChecked}/></div>; }
